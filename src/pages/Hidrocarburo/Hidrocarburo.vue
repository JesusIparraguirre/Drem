...
<div class="mineria-page">
  <b-row>
    <b-col cols="8">
      <h2 class="page-title">Area - <span class="fw-semi-bold">Hidrocarburo</span></h2>
    </b-col>
            ...
            <table class="table" v-if="expedientes.length != 0">
              <thead>
                <tr>
                  ...
                  <th class="hidden-sm-down" v-if="user.id_area_trabajo == 8 || user.id_area_trabajo == 2">Aprobar</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in expedientes" :key="row.id_expediente" >
                  ...
                  <td v-if="user.id_area_trabajo == 8 || user.id_area_trabajo == 2">
              ...
              <b-form @submit.prevent="onSubmit" @reset.prevent="onReset" v-if="show">
                ...
                <b-form-group v-show="csObservaciones==true" id="input-group-4" label="Informe de observaciones:" label-for="input-4">
                  <div class="text-right">
                    ...
                    :options="dropzoneOptions" @vdropzone-complete="afterCompleteObservaciones" @uploadprogress="cargando">
                    </vue-dropzone>
                  </div>
                </b-form-group>
...
export default {
  ...
  methods: {
    ...
    loadAll(){
      this.loadingTable = true;
      this.showTable = false;
      axios.get(config.hostname+'expediente/obtener_expediente_area/2')
    .then( (response) =>{
      this.expedientes = response.data.data;
      this.loadingTable = false;
      this.showTable = true;
    ...
    loadData(){
      axios.get(config.hostname+'expediente/obtener_expediente_area/2/'+this.page+'/'+this.perpage)
      .then( (response) =>{
        this.loadingTable = false;
        this.showTable = true;
    ...
    loadPerPage(perpage){
      this.page = 1;
      axios.get(config.hostname+'expediente/obtener_expediente_area/2/1/'+perpage)
      .then( (response) =>{
        this.expedientes = response.data.data;
      })
      .catch( (error) =>{
    ...
    loadPage(page){
      axios.get(config.hostname+'expediente/obtener_expediente_area/2/'+page+'/'+this.perpage)
      .then( (response) =>{
...
<style src="./Hidrocarburo.scss" lang="scss" scoped />