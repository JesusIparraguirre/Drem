...
<div class="mineria-page">
  <b-row>
    <b-col cols="8">
      <h2 class="page-title">Area - <span class="fw-semi-bold">Minería</span></h2>
    </b-col>
            ...
            <table class="table" v-if="expedientes.length != 0">
              <thead>
                <tr>
                  ...
                  <th class="hidden-sm-down">Mercurio</th>
                  <th class="hidden-sm-down">Estado</th>
                  <th class="hidden-sm-down" v-if="user.id_area_trabajo == 8 || user.id_area_trabajo == 3">Aprobar</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in expedientes" :key="row.id_expediente" >
                  ...
                  <td class="text-semi-muted">
                     <div style="width: 120px;white-space: normal;overflow: hidden;text-overflow: Ellipsis" v-for="file in row.arc_exp" :key="file.id_archivos_exp">
                        <a target="_blank" :href="ip+'/public/'+file.c_archivo_exp">
                        {{file.c_archivo_exp}}</a>
                      </div>
                  </td>
                  <td class="text-semi-muted">
                     <div style="width: 120px;white-space: normal;overflow: hidden;text-overflow: Ellipsis" v-for="file in row.arc_exp_obs" :key="file.id_archivos_obs_exp">
                  ...
                  <td class="text-semi-muted">
                     <!-- <div v-for="file in row.arc_exp" :key="file.id_archivos_exp">
                        <a target="_blank" :href="config.ip+'/public/'+file.c_archivo_exp">
                        {{file.c_archivo_exp}}</a>
                      </div> -->
                  </td>
                  ...
                  <td v-if="user.id_area_trabajo == 8 || user.id_area_trabajo == 3">
          ...
          <div v-show="showDetail">
            ...
            <div class="widget-padding-md w-100 h-100 text-left border rounded">
              <b-form @submit.prevent="onSubmit" @reset.prevent="onReset" v-if="show">
                ...
                <b-form-group v-show="csObservaciones==false" id="input-group-4" label="Informe Técnico:" label-for="input-4">
                  <div class="text-right">
                 ...
                 <b-form-group v-show="csObservaciones==false" id="input-group-4" label="AutoDirectoral:" label-for="input-4">
                  <div class="text-right">
                    ...
                    <vue-dropzone ref="dropzoneAutoDirectoral" id="dropzoneAutoDirectoral" 
                    :options="dropzoneOptions" @vdropzone-complete="afterCompleteAutoDirectoral"></vue-dropzone>
                  </div>
                </b-form-group>
                <b-form-group  v-show="csObservaciones==false"  id="input-group-4" label="Declaración de Impacto Ambiental:" label-for="input-4">
                  <div class="text-right">
                    <b-button @click="removeAmbiental" variant="danger">Quitar todo</b-button>
                    <vue-dropzone ref="dropzoneAmbiental" id="dropzoneAmbiental" 
                    :options="dropzoneOptions" @vdropzone-complete="afterCompleteAmbiental"></vue-dropzone>
                  </div>
                </b-form-group> 
...
export default {
  ...
  methods: {
    ...
    loadAll(){
      this.loadingTable = true;
      this.showTable = false;
      axios.get(config.hostname+'expediente/obtener_expediente_area/3/'+this.page+'/'+this.perpage)
      .then( (response) =>{
      this.expedientes = response.data.data;
      this.loadingTable = false;
      this.showTable = true;
    ...
    loadPerPage(perpage){
      this.page = 1;
      axios.get(config.hostname+'expediente/obtener_expediente_area/3/1/'+perpage)
      .then( (response) =>{
        this.expedientes = response.data.data;
      })
      .catch( (error) =>{
    ...
    loadPage(page){
      // this.PageUp();
      axios.get(config.hostname+'expediente/obtener_expediente_area/3/'+page+'/'+this.perpage)
      .then( (response) =>{
    ...
    onSubmit(){
      if(this.csObservaciones == true){
        if(this.fileObservaciones.length == 0){
         ...
         if(this.fileInformeTecnico.length == 0 || this.fileAutoDirectoral.length == 0 || 
         this.fileAmbiental.length == 0){
           ...
           this.loadingForm = true;
            ...
            this.fileAmbiental.forEach(
              e=> fd.append('c_decla_impac_amb',e),
            );
            fd.append('id_expediente',this.form.id_expediente);
...
<style src="./Mineria.scss" lang="scss" scoped />